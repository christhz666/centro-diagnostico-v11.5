...
class ApiService {
    ...
    async login(credentials) {
        ...
        if (!response.ok) {
            ...
            throw new Error(errData.message || errData.error || 'Credenciales inválidas');
        }
    ...
    async getDashboardStats() {
        ...
        if (!raw.ok) throw new Error('Error cargando estadísticas');
    ...
    async getCitasHoy() {
        ...
        if (!raw.ok) throw new Error('Error cargando citas de hoy');
        const data = await raw.json();
    ...
    async cambiarEstadoCita(id, estado, notas) {
        return this.request('/citas/' + id + '/estado', {
            method: 'PATCH', body: JSON.stringify({ estado, notas })
        });
    }
    ...
    async anularFactura(id, motivo) {
        return this.request('/facturas/' + id + '/anular', { method: 'PATCH', body: JSON.stringify({ motivo }) });
    }
    ...
    async validarResultado(id, data) {
        return this.request('/resultados/' + id + '/validar', { method: 'PATCH', body: JSON.stringify(data) });
    }
