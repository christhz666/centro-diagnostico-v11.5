...
function LogoUploader({ label, descripcion, fieldKey, value, onChange }) {
  ...
  const limpiar = () => { setPreview(''); onChange(fieldKey, ''); if (inputRef.current) inputRef.current.value = ''; };
...
const AdminPanel = () => {
  ...
  const botonGuardar = (texto) => (
    <button type="submit" disabled={guardando} style={{
      ...
      {guardando ? 'Guardando...' : guardado ? '¡Guardado Correctamente!' : texto}
    </button>
  );
        ...
        <form onSubmit={guardar}>
          ...
          <Seccion titulo="Logos del Sistema" icono={<FaImage style={{ color: '#8e44ad' }} />}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '0 24px' }}>
              <LogoUploader label="🔐 Logo del Panel de Inicio de Sesión" descripcion="Aparece en la pantalla de login. Tamaño ideal: 300×100px." fieldKey="logo_login" value={config.logo_login} onChange={set} />
              <LogoUploader label="🧾 Logo de Facturas" descripcion="Se imprime en la parte superior de cada factura térmica. Tamaño ideal: 250×80px." fieldKey="logo_factura" value={config.logo_factura} onChange={set} />
              <LogoUploader label="📋 Logo de Resultados" descripcion="Aparece en los reportes de resultados de laboratorio e imagenología. Tamaño ideal: 300×100px." fieldKey="logo_resultados" value={config.logo_resultados} onChange={set} />
              <LogoUploader label="📌 Logo de la Barra Lateral (Sidebar)" descripcion="Aparece en la parte superior del menú lateral. Tamaño ideal: 160×50px." fieldKey="logo_sidebar" value={config.logo_sidebar} onChange={set} />
            </div>
          </Seccion>
